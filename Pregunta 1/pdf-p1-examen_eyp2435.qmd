...
R_2 = cbind( c(summary_mod$`Response fertility`$r.squared, summary_mod$`Response fertility`$adj.r.squared), 
             c(summary_mod$`Response lifeExpF`$r.squared, summary_mod$`Response lifeExpF`$adj.r.squared))

knitr::kable(R_2, digits = 3, aline = "c")
```

Analizando si el modelos es adecuado podemos observar que el $R^2$ se ajusta correctamente, en específico el $R^2$ de *lifeExpF* se ajusta de mejor forma que de *fertility*. Pero ambos son con un valor mayor que 0.5, lo que nos indica que efectivamente este modelo es adecuado para ambas variables.
