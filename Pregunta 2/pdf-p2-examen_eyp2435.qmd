...
En base a los indicadores útiles para análisis obtenidos en la encuesta socioeconómica realizada en el 2017 por la Casen, intentaremos caracterizar a la población pobre y no pobre mediante un análisis factorial. Además, intentaremos utilizar una regresión lineal con estos factores para predecir el ingreso per cápita de los distintos hogares de Chile.

# Introducción

Al momento de realizar sus encuestas, la Casen tiene dos grandes objetivos en mente: el primero, es el de conocer la situación en que se encuentran las personas, sobre todo quienes se encuentran en situación de pobreza o en algún estado vulnerable o que los vuelva políticamente prioritarios (como los adultos mayores, niños, etc.); mientras que el segundo va de la mano con estimar el impacto de los gastos fiscales en el ingreso de los hogares. 

Es por esto que, teniendo en cuenta el primer objetivo de la encuesta, en el presente informe buscaremos caracterizar a las personas de la población, centrándonos específicamente en aquellas que se encuentran en situación de pobreza. De esta manera, nos hemos planteado dos hipótesis que buscaremos desmentir o comprobar por medio del análisis estadístico de los datos:

1. Mediante análisis factorial y de componentes principales, es posible generar un modelo de regresión logística que nos ayude a diferenciar a las personas en situación de pobreza de las que no lo están.
2. Mediante análisis factorial y de componentes principales, es posible generar un modelo de regresión lineal que nos permita estimar de manera adecuada los ingresos autónomos per cápita de los hogares chilenos.

Así, realizaremos una breve descripción de los datos con los que vamos a trabajar, mencionaremos y justificaremos las metodologías que vamos a utilizar, mostraremos los resultados a los que llegamos y, finalmente, veremos si nuestras ideas estaban en lo cierto o no.
...
Para este analisis usaremos 2 medidas de los ingresos, la media de estos y la mediana. Debido a que la media está sesgada por los valores extremos, en este caso las personas que tienen ingresos muy altos afectan al promedio, es por ello que también usaremos la mediana. De @fig-3-1 logramos divisar que la mayor media de los ingresos se encuentra principalmente en la región Metropolitana (XIII) con un ingreso promedio mayor de 400.000 pesos, seguida de la región de Magallanes (XII). Pero analizando la @fig-3-2 donde se presentan las medianas de los ingreso autónomo per cápita del hogar corregido de las personas por región podemos ver que la región de Magallanes (XII) presenta mayor mediana de ingresos (más de 240.000 pesos), mientras que la región Metropolitana (XIII) tiene una mediana de ingresos entre 200.000 pesos y 240.000 pesos. Con este caso de la región Metropolitana podemos analizar esta región para tener una visión de lo que pasa al interior de esta.
...
Ahora usaremos los 8 componentes principales para generar una regresión lineal para poder estimar los ingreso autónomo per cápita del hogar corregido. Luego de crear este modelo con los 8 componentes principales, compararemos con modelos sin alguno de estos componentes y compararemos los AIC de los modelos, tal como muestra @fig-20.
...
Además, se comprobó que no es posible realizar un análisis de regresión lineal, el cual intenta explicar los ingreso autónomo per cápita del hogar corregido en Chile. Al analizar los ingresos se observa que existe una importante cantidad de ingresos muy elevados, los cuales aumentan los residuos del modelo lineal.
